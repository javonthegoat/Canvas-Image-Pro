...
- **New Image Placement**: New images are now added to the top of the layer stack.
...
- **Drag and Drop Annotations**: Drag and drop annotations from images in the layers panel into other images, or from the canvas onto images. Annotations can also be dragged out of images onto the canvas.
...
- **Grouping**: Select multiple images and group them together. Images in the layers panel can now be dragged into groups. Images that are already in groups can be dragged out of groups from the layer stack, and images inside of groups can be dragged into other groups from the layers panel.
...
- **Arrange Grid**: Arrange images in a grid. The order is based on the layer stack (top-most to bottom-most by default). Clicking the arrange button again reverses the order.
- **Arrange Stack**: Stack images horizontally or vertically in a single line. The order is based on the layer stack (top-most to bottom-most by default). Clicking the stack button again reverses the order.
...
- **Copy and Paste Annotations**: Select one or more annotations and press `Ctrl + C` to copy them. Press `Ctrl + V` to paste them. The annotations will be pasted at the location of the mouse cursor.

### History and Layer Management

- **Undo and Redo**: Use `Ctrl + Z` to undo and `Ctrl + Y` to redo your actions.
- **Layer Reordering**: In the layers panel, you can drag and drop layers to reorder them. You can also use the buttons to move layers up, down, to the top, or to the bottom of the stack.
